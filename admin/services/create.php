...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($name) || empty($slug)) {
        ...
        try {
            $stmt = $pdo->prepare("INSERT INTO services (name, slug, description, icon, status) VALUES (:name, :slug, :description, :icon, :status)");
            $stmt->execute([
                ':name' => $name,
                ':slug' => $slug,
...
<form method="POST" action="" class="app-card" style="max-width: 800px;">
    ...
    <div class="grid" style="grid-template-columns: repeat(auto-fit, minmax(250px, 1fr)); gap: 20px;">
        <div style="margin-bottom: 15px;">
            ...
            <input type="text" name="icon" value="bx-briefcase" placeholder="bx-briefcase" style="width: 100%; padding: 12px; border-radius: 8px; border: 1px solid var(--glass-border); background: var(--bg-primary); color: var(--text-primary); outline: none;">
        </div>
        <div style="margin-bottom: 15px;">
    ...
    <div style="margin-top: 30px;">
        <button type="submit" class="btn btn-primary" style="padding: 15px 30px; font-size: 1.1rem;">Save Service</button>
    </div>
</form>
